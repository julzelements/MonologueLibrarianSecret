...
describe("Utilities tests", () => {
  describe("bin()", () => {
    it("returns 8 bit zero-padded binary representation by default", () => {
      ...
      expect(bin(testValue)).toEqual(expectedResponse);
    });
  ...
  describe("getBits()", () => {
    it("returns the number represented by the binary digits at a mid section slice", () => {
      ...
      expect(getBits(testValue, start, end)).toEqual(expectedResponse);
    });
  ...
  describe("addLowerBits()", () => {
    it("correctly returns 10-bit number from 2 8-bit numbers using last two digits of second as low bits", () => {
      ...
      expect(addLowerBits(testValueHighBits, testValueLowBits, 0)).toEqual(
        expectedResponse
      );
    });
    it("correctly returns 10-bit number from 2 8-bit numbers using arbitrary two digits of second as low bits", () => {
      ...
      expect(addLowerBits(testValueHighBits, testValueLowBits, 4)).toEqual(
        expectedResponse
      );
    });
  });
  describe("addHighBit()", () => {
    it("correctly returns 8-bit number from 2 8-bit numbers using last digit of second to replace high bit of first", () => {
      ...
      expect(addHighBit(testValueLowBits, testValueHighBit, 0)).toEqual(
        expectedResponse
      );
    });
  ...
  describe("transformDataFrom7BitTo8Bit()", () => {
    it("correctly transforms the test data", () => {
      const records = [
        /* headers */ 240, 66, 49, 0, 1, 68, 64, /* set 1 */ 0, 80, 82, 79, 71,
        73, 110, 105, /* set 2 */ 84, 97, 109, 0, 127, 0, 0, 127, 247,
      ];
      const expectedResponse = [
        /* set 1 unchanged */ 80, 82, 79, 71, 73, 110, 105,
        /* set 2 updated*/ 97, 109, 128, 127, 128, 0, 255,
      ];
      /*
      Set 2 updates:
      ...
      */
      expect(transformDataFrom7BitTo8Bit(records)).toEqual(expectedResponse);
    });
  });
});
